...
%
% Description: 
%  Function to solve a boundary layer problem for the linear advection-diffusion equation 
%  using a multi-block FSBP-SAT method. 
...
% x_ref :       local grid points in the reference element 
...
function [ x_global, u_num, mass, energy ] = solve_boundaryLayer( a, epsilon, x_L, x_R, T, u_init, I, D1, D2, x_ref, P )
    ...
    %dt = 10^(-3)*(dx_min*block_width); % time-step size
    ...
    while (t<T)  
        ...
        for i = 1:I
            ...
            if i==1 % if we are considering the first element 
                u_L = zeros(N,1); % we take the last element, assuming periodic BC 
                Du_L(N) = 1 / ( epsilon*( exp(.5/epsilon) - 1 ) ); 
            else % otherwise 
                u_L = u(:,i-1); % we take the element to the left-hand side
                Du_L = D1*u_L; % its derivative 
            end

            % solution in the element to the right-hand side 
            if i==I % if we are considering the last element 
                u_R = ones(N,1); % we take the first element, assuming periodic BC; 
                Du_R(1) = exp(.5/epsilon) / ( epsilon*( exp(.5/epsilon) - 1 ) ); 
            else % otherwise 
                u_R = u(:,i+1); % we take the element to the right-hand side
                Du_R = D1*u_R; % its derivative 
            end
